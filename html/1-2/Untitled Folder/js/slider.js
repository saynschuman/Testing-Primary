$(document)
    .ready(function () {
        $('.about-lnk')
            .click(function () {
                ...
                var isvisible = $(this)
                    ...
                    .hasClass('open');
                $(this)
                    .html(isvisible ? 'Скрыть' : 'Показать весь текст');
            })
        $('.lte9 .promo-item:last-child')
            .css({
                'margin-right': '0'
            })
        $('#data-giftSumm')
        ...
        $('.data-GiftChoice')
            .on('click', function () {
                var price = $('#data-giftSumm')
                    .val();
                if (price) {
                    document.location.href = 'http://590.ua/catalog/gifts/price/' + price;
                } else {
                    $('#data-giftSumm')
                        .attr('data-original-title', 'Введите сумму');
                    $('#data-giftSumm')
                        .tooltip('show')
                }
            });
        ...
        var billboardSize = $('.billboard li')
            .size();
        var billboardGallery = $('.billboard .img-small');
        var billboardIndex = 0;
        var controlLeft = $('.billboard-control__left');
        var controlRight = $('.billboard-control__right');
        billboardGallery.css({
            'width': billboardSize * 150
        });
        $('.billboard li')
            .on('click', function () {
                if ($(this)
        ...
        function getBannerPos() {
            (billboardIndex == 0) ? controlLeft.hide(): controlLeft.show();
            ((billboardSize - billboardIndex) == 5) ? controlRight.hide(): controlRight.show();
        }
        getBannerPos();
        $('.billboard-control__left')
            .on('click', function () {
                if (billboardIndex == 0) {
                    return;
                } else {
                    billboardIndex--;
                    billboardGallery.animate({
                        left: "+=150"
                    , }, getBannerPos);
                }
            });
        $('.billboard-control__right')
            .on('click', function () {
                if (billboardIndex >= (billboardSize - 5)) {
                    return;
                } else {
                    billboardIndex++;
                    billboardGallery.animate({
        ...
        , refresh: function () {
            ...
            $targets = this.$body.find(this.selector)
                .map(function () {
                    ...
                    return ($href && $href.length && [[$href.position()
                        .top, href]]) || null
                })
                .sort(function (a, b) {
                    return a[0] - b[0]
...
(function ($, undefined) {
    ...
    function getCursorInfo(el) {
        ...
        if ('selectionStart' in el) {
            ...
            length = document.selection.createRange()
                .text.length;
    ...
    AbstractSelect2 = clazz(Object, {
            ...
            if ($.fn.mousewheel) {
                results.mousewheel(function (e, delta, deltaX, deltaY) {
                    ...
                    if (deltaY > 0 && top - deltaY <= 0) {
                        results.scrollTop(0);
                        killEvent(e);
                    } else if (deltaY < 0 && results.get(0)
                        .scrollHeight - results.scrollTop() + deltaY <= results.height()) {
                        results.scrollTop(results.get(0)
                            .scrollHeight - results.height());
                        killEvent(e);
                    }
                });
            }
        ...
        , destroy: function () {
            ...
            if (select2 !== undefined) {
                ...
                select2.opts.element.removeClass("select2-offscreen")
                    .removeData("select2")
                    .off(".select2")
                    .attr({
                    ...
                    if ("ajax" in opts) {
                        ...
                        if (opts.initSelection === undefined) {
                            opts.initSelection = function (element, callback) {
                                var data = [];
                                $(splitVal(element.val(), opts.separator))
                                    .each(function () {
                                        var id = this
                                            , text = this
                                            , tags = opts.tags;
        ...
        , positionDropdown: function () {
            var $dropdown = this.dropdown
                ...
                , viewPortRight = $(window)
                .scrollLeft() + $(window)
                .width()
                , viewportBottom = $(window)
                .scrollTop() + $(window)
                .height()
                , dropTop = offset.top + height
                , dropLeft = offset.left
                , enoughRoomBelow = dropTop + dropHeight <= viewportBottom
                , enoughRoomAbove = (offset.top - dropHeight) >= this.body()
                .scrollTop()
                , dropWidth = $dropdown.outerWidth(false)
                , enoughRoomOnRight = dropLeft + dropWidth <= viewPortRight
                , aboveNow = $dropdown.hasClass("select2-drop-above")
                , bodyOffset, above, css, resultsListNode;
        ...
        , opening: function () {
            ...
            if (mask.length == 0) {
                ...
                mask.attr("id", "select2-drop-mask")
            ...
            this.container.parents()
                ...
                .each(function () {
                    $(this)
                        .on(resize + " " + scroll + " " + orient, function (e) {
                            $("#select2-drop-mask")
                                .css(_makeMaskCss());
            ...
            function _makeMaskCss() {
                return {
                    width: Math.max(document.documentElement.scrollWidth, $(window)
                        .width())
                    , height: Math.max(document.documentElement.scrollHeight, $(window)
                        .height())
                }
            }
        }
        ...
        , ensureHighlightVisible: function () {
            ...
            hb = child.offset()
                .top + child.outerHeight(true);
            if (index === children.length - 1) {
                more = results.find("li.select2-more-results");
                if (more.length > 0) {
                    hb = more.offset()
                        .top + more.outerHeight(true);
                }
            }
            rb = results.offset()
                .top + results.outerHeight(true);
            if (hb > rb) {
        ...
        , updateResults: function (initial) {
            ...
            if (search.val()
                .length < opts.minimumInputLength) {
            ...
            if (opts.maximumInputLength && search.val()
                .length > opts.maximumInputLength) {
            ...
            if (opts.formatSearching && this.findHighlightableChoices()
                .length === 0) {
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        , setPlaceholder: function () {
            ...
            if (this.opts.element.val() === "" && placeholder !== undefined) {
                if (this.select && this.select.find("option:first")
                    .text() !== "") return;
                this.selection.find("span")
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        , initContainer: function () {
            ...
            this.search.on("keydown", this.bind(function (e) {
                ...
                if (selected.length && (e.which == KEY.LEFT || e.which == KEY.RIGHT || e.which == KEY.BACKSPACE || e.which == KEY.DELETE || e.which == KEY.ENTER)) {
                    ...
                    this.selectChoice(selection.find(".select2-search-choice:not(.select2-locked)")
                        .last());
                    killEvent(e);
                    return;
                } else {
        ...
        , clearSearch: function () {
            ...
            if (placeholder !== undefined && this.getVal()
                .length === 0 && this.search.hasClass("select2-focused") === false) {
                this.search.val(placeholder)
            ...
            if (enableChoice) {
                choice.find(".select2-search-choice-close")
                    ...
                    .on("click dblclick", this.bind(function (e) {
                        ...
                        $(e.target)
                            .closest(".select2-search-choice")
                            .fadeOut('fast', this.bind(function () {
        ...
        , postprocessResults: function (data, initial, noHighlightUpdate) {
            ...
            choices.each2(function (i, choice) {
                ...
                if (indexOf(id, val) >= 0) {
                    ...
                    choice.find(".select2-result-selectable")
                        .addClass("select2-selected");
                }
            });
            compound.each2(function (i, choice) {
                if (!choice.is('.select2-result-selectable') && choice.find(".select2-result-selectable:not(.select2-selected)")
                    .length === 0) {
                    choice.addClass("select2-selected");
                }
            });
            ...
            if (!this.opts.createSearchChoice && !choices.filter('.select2-result:not(.select2-selected)')
                .length > 0) {
        ...
        , resizeSearch: function () {
            ...
            left = this.search.offset()
                .left;
            maxWidth = this.selection.width();
            containerLeft = this.selection.offset()
                .left;
    ...
    $.fn.select2 = function () {
        var args = Array.prototype.slice.call(arguments, 0)
            , opts, select2, value, multiple, allowedMethods = ["val", "destroy", "opened", "open", "close", "focus", "isFocused", "container", "onSortStart", "onSortEnd", "enable", "readonly", "positionDropdown", "data"]
    ...
    var methods = {
            ...
            if (!options.binded) {
                if (options.bindMethod == "bind") {
                    form.find("[class*=validate]")
                        ...
                        .not("[type=radio]")
                        .not(".datepicker")
                        .bind(options.validationEventTrigger, methods._onFieldEvent);
                    form.find("[class*=validate][type=checkbox],[class*=validate][type=radio]")
                        .bind("click", methods._onFieldEvent);
                    form.find("[class*=validate][class*=datepicker]")
                        .bind(options.validationEventTrigger, {
                            "delay": 300
                    ...
                    form.find("[class*=validate]")
                        .not("[type=checkbox]")
                        .not(".datepicker")
                        .live(options.validationEventTrigger, methods._onFieldEvent);
                    form.find("[class*=validate][type=checkbox]")
                        .live("click", methods._onFieldEvent);
                    form.find("[class*=validate][class*=datepicker]")
                        .live(options.validationEventTrigger, {
                            "delay": 300
                        }, methods._onFieldEvent);
        ...
        , detach: function () {
            ...
            if (options.binded) {
                form.find("[class*=validate]")
                    .not("[type=checkbox]")
                    .unbind(options.validationEventTrigger, methods._onFieldEvent);
                form.find("[class*=validate][type=checkbox],[class*=validate][type=radio]")
                    .unbind("click", methods._onFieldEvent);
                form.unbind("submit", methods.onAjaxFormComplete);
                form.find("[class*=validate]")
                    .not("[type=checkbox]")
                    .die(options.validationEventTrigger, methods._onFieldEvent);
                form.find("[class*=validate][type=checkbox]")
                    .die("click", methods._onFieldEvent);
                form.die("submit", methods.onAjaxFormComplete);
                form.removeData('jqv');
                if (options.autoPositionUpdate) {
                    $(window)
                        .unbind("resize", methods.updatePromptsPosition)
                }
            }
            return this;
        }
        ...
        , hidePrompt: function () {
            var promptClass = "." + methods._getClassName($(this)
                .attr("id")) + "formError";
            $(promptClass)
        ...
        , hide: function () {
            ...
            if ($(this)
                .is("form")) {
                closingtag = "parentForm" + methods._getClassName($(this)
                    .attr("id"));
            } else {
                closingtag = methods._getClassName($(this)
                    .attr("id")) + "formError";
            }
            $('.' + closingtag)
                .fadeTo("fast", 0.3, function () {
        ...
        , _validateFields: function (form, skipAjaxValidation) {
            ...
            if (errorFound) {
                if (options.scroll) {
                    var destination = first_err.offset()
                        .top;
                    var fixleft = first_err.offset()
                        .left;
                    ...
                    if (options.isOverflown) {
                        ...
                        var scrollContainerPos = -parseInt(overflowDIV.offset()
                            .top);
        ...
        , _validateField: function (field, options, skipAjaxValidation) {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                case "groupRequired":
                    ...
                    var firstOfGroup = form.find(classGroup)
                        .eq(0);
                ...
                case "dateRange":
                    ...
                    var firstOfGroup = form.find(classGroup)
                        .eq(0);
                    var secondOfGroup = form.find(classGroup)
                        .eq(1);
            ...
            if ((fieldType == "radio" || fieldType == "checkbox") && form.find("input[name='" + fieldName + "']")
                .size() > 1) {
                field = $(form.find("input[name='" + fieldName + "'][type!=hidden]:first"));
                options.showArrow = false;
            }
            if (fieldType == "text" && form.find("input[name='" + fieldName + "']")
                .size() > 1) {
                field = $(form.find("input[name='" + fieldName + "'][type!=hidden]:first"));
                options.showArrow = false;
            }
        ...
        , _buildPrompt: function (field, promptText, type, ajaxed, options) {
            ...
            if (field.is(":input")) prompt.addClass("parentForm" + methods._getClassName(field.parents('form')
                .attr("id")));
...
(function (h) {
    ...
    var e = function (j) {
        var i = j.keyCode;
        i = h.browser.safari ? (a[i] || i) : i;
        return (i >= 33 && i <= 40) || i == c.RETURN || i == c.TAB || i == c.ESC
    };
    var d = function (j) {
        var i = j.keyCode;
        var l = j.charCode;
        return i == 9 || i == 13 || (i == 40 && (!h.browser.opera || !j.shiftKey)) || i == 27 || i == 16 || i == 17 || (i >= 18 && i <= 20) || (h.browser.opera && !j.shiftKey && (i == 8 || (i >= 33 && i <= 35) || (i >= 36 && i <= 39) || (i >= 44 && i <= 45)))
    };
    var b = function (j) {
        var i = j.keyCode || j.charCode;
        return h.browser.safari ? (a[i] || i) : i
    };
    var g = function (i) {
        return i.charCode || i.keyCode || i.which
    };
    h.fn.keyfilter = function (i) {
        return this.keypress(function (m) {
            if (m.ctrlKey || m.altKey) {
                return
            }
            var j = b(m);
            if (h.browser.mozilla && (e(m) || j == c.BACKSPACE || (j == c.DELETE && m.charCode == 0))) {
                return
            }
            var o = g(m)
                , n = String.fromCharCode(o)
                , l = true;
            if (!h.browser.mozilla && (d(m) || !n)) {
                return
            }
            if (h.isFunction(i)) {
                l = i.call(this, n)
    ...
    h(document)
        .ready(function () {
            var i = h("input[class*=mask],textarea[class*=mask]");
            for (var j in h.fn.keyfilter.defaults.masks) {
                i.filter(".mask-" + j)
                    .keyfilter(h.fn.keyfilter.defaults.masks[j])
            }
        })
})(jQuery);
(function ($) {
    ...
    var methods = {
        init: function (data) {
            ...
            $('.' + options.modal_error_class)
                .live('click', function (e) {
                    $(this)
                        .removeClass(options.modal_error_class);
                    $(this)
                        .hide();
                    el.val('');
                    el.focus();
...
function Menu() {
    ...
    if ($('#sidebar1 .navigation li.active')
        .size() == 0) {
        $('#sidebar1 .navigation')
            .children('li')
            .first()
...
function filter(id) {
    ...
    if (id != 'price') {
        for (var i in arr) {
            if (arr[i] == 'filter' + id + '_from') {
                arr[parseInt(i) + 1] = $('#amount_from_' + id)
                    .val() != '' ? $('#amount_from_' + id)
                    .val() : 0;
                var found = true;
            }
            if (arr[i] == 'filter' + id + '_to') {
                arr[parseInt(i) + 1] = $('#amount_to_' + id)
                    .val() != '' ? $('#amount_to_' + id)
                    .val() : 0;
                var found = true;
            }
    ...
    if (!found) {
        if (id != 'price') {
            ...
            var val = $('#amount_from_' + id)
                .val() != '' ? $('#amount_from_' + id)
                .val() : 0;
            arr.push(val);
            arr.push('filter' + id + '_to');
            var val = $('#amount_to_' + id)
                .val() != '' ? $('#amount_to_' + id)
                .val() : 0;
            arr.push(val);
        } else {
...
function rotateBanner() {
    ...
    if ($(el)
        ...
        $.getJSON('/banners/getbanner', {
            ...
            $(el)
                .find('img')
                .attr('src', '/public/uploads/' + data.filename);
        });
...
function getCompareItems() {
    $.get("/catalog/getcompareitem", {}, function (data) {
        if (data.length > 1) {
            $(".data-navbar")
                .show();
            $(".data-compareItems")
...
function sendCallback() {
    ...
    phone = $("#asked-phone")
        .val();
    jQuery.get("/catalog/callback", {
        name: name
        , phone: phone
        , id_item: window.id_item
    }, function () {
        $('#asked')
            .modal('hide')
    });
...
$(document)
    .ready(function () {
        ...
        $('.data-openBasket')
            .on('click', function (e) {
                e.preventDefault();
                $.get('/basket/basket-data', {}, function (data) {
                    $('.data-basket')
                        .html(data);
                    $(".data-amount")
                        .keyfilter(/[\d\-]/);
        ...
        $("#login-form, #callback-form, .form-search")
            .validationEngine();
        if ($.browser.msie)
            $("#phone")
        ...
        $('#data-sendError')
            .live('click', function () {
                var text = $('#data-errorText')
                    .html();
                var url = $('#data-errorUrl')
                    .val();
                var comments = $('#data-errorComments')
                    .val();
                $.getJSON("/texterror", {
                    text: text
                    , url: url
                    ...
                    $('#error-modal #data-errorText')
                        .html('Спасибо за обнаруженную ошибку');
                    $('#data-sendError, #data-errorComments')
                        .remove();
                });
            });
        $(document)
            .keypress(function (e) {
                if (e.ctrlKey && ((e.keyCode == 0xA) || (e.keyCode == 0xD))) {
                    ...
                    $('#error-modal .modal-header h3')
                        .html('Вы обнаружили ошибку в тексте:');
                    $('#error-modal .modal-body cite')
                        .html(text);
                    $('#data-errorUrl')
                        .val(url);
                }
        ...
        if ($("#stext")
            ...
            var arr = $("#stext")
                .html()
                .split('.');
            var pf = '<div id="fst">' + arr[0] + '.</div>';
            arr.splice(0, 1);
            var pe = '<div id="etxt">' + arr.join('.') + '</div>';
            $("#stext")
                .html(pf + pe);
            $('#etxt')
                .toggle();
        ...
        $('#callback-form')
            .live('submit', function (e) {
                ...
                request['message'] = $(this)
                    .find('#message')
                    .val();
                $.post('/callback', request, function (data) {
                    $('#callback .modal-body')
                        .html('Спасибо, с Вами свяжется менеджер интернет-магазина в течение рабочего дня. Мы перезваниваем 6 дней в неделю, кроме воскресенья: понедельник - пятница: 10.00 - 18.00; суббота: 11.00 - 17.00.');
                    $('#data-sendCallback')
                        .remove();
                });
            });
        ...
        $(".data-remove")
            .live("click", function () {
                ...
                $.get("/basket/delitem", getvars, function (data) {
                    refreshBasketStats();
                });
                $(this)
                    .closest('tr')
            ...
            .live("keyup", function (event) {
                if ($(this)
                    ...
                    if ($(this)
                        ...
                        .val())
                        getvars.type = 'set';
                    $.get("/basket/change", getvars, function (data) {
                        el.closest('tr')
                            .find('.data-itemPrice')
                            .html(price * newval);
                        refreshBasketStats();
        ...
        $('.data-toggleCompare')
            .on('click', function () {
                var $this = $(this);
                if ($this.hasClass('checked') || $('.data-cloneImg')
                    .is(':animated')) return;
                $img = $('<img>', {
                    'class': 'data-cloneImg'
                    , 'src': $this.data('url')
                    ...
                    , function (data) {
                        if (data.result) {
                            $('#comments-modal .modal-body')
                                .html('Спасибо за Ваш отзыв. Он появится на сайте после проверки администратором.');
                            $('#comments-modal .data-login')
                                .remove();
                        }
                    });
            });
        ...
        $('.product_box')
            .each(function () {
                var container = '.' + $(this)
                    .find('.products_container_wrapper')
                    .children()
                    .first()
                    .attr('class');
                var childrens = $(container)
                    .children()
                    .length;
                var width = $(container)
                    .children()
                    .first()
                    .outerWidth(true);
                var container_width = width * childrens;
                $(container)
                    .css('width', container_width + 'px');
            })
        window.anim_status = false;
    });

function next_product(that, count) {
    if (!window.anim_status) {
        window.anim_status = true;
        var count = count + 1;
        var container = $(that)
            .parents('.product_box')
            .find('.products_container_wrapper')
        ...
        var first_block = $(container)
            .children()
            .first();
        var first_block_class = '.' + $(container)
            .children()
            .first()
            .attr('class');
        var width = $(first_block)
            .outerWidth(true);
        var last_element = $(container)
            .find(first_block_class + ':nth-child(' + count + ')');
        $(last_element)
            .css('opacity', '0');
        $(container)
            .animate({
...
function prev_product(that, count) {
    if (!window.anim_status) {
        ...
        var container = $(that)
            ...
            .children()
            .first();
        var last_block = $(container)
            .children()
            .last();
        var width = $(last_block)
            .width();
        $(last_block)
            .hide()
        $(container)
            ...
            return this.each(function () {
                ...
                function checkVal(allow) {
                    ...
                    if (!allow && lastMatch + 1 < partialPosition) {
                        ...
                        writeBuffer();
                        if (!allow) input.val(input.val()
                            .substring(0, lastMatch + 1));
                    }
    ...
    .ready(function () {
        $('.data-zoom')
            .live('click', function () {
                ...
                var zoom = $("<div>", {
                    'class': "zoom"
                    , 'html': "<img src = " + imgUrl + " /><span><i>" + imgTitle + "</i></span>"
                    , 'css': {
                        'width': w
                        , 'height': h
                        , 'line-height': h + 'px'
